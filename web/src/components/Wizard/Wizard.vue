    ...
    <div class="wizard-container">
        <div class="card card-wizard active mb-0" data-color="primary">
            <form @submit.prevent>
                ...
                <div class="card-header text-center bg-hintify">
                    <slot name="header">
                    ...
                    <div class="wizard-navigation">
                        <ul class="nav nav-pills bg-hintify" role="tablist">
                ...
                <div class="card-footer">
                    <slot name="footer"
                        ...
                        <div class="pull-right">
                            <n-button v-if="activeTabIndex < tabCount -1"
                                      wide
                                      @click.native="nextTab"
                                      class="btn-next">
                                {{nextButtonText}}
                            </n-button>
                            <n-button v-else wide @click.native="nextTab">{{finishButtonText}}</n-button>
                        </div>

                        <div class="pull-left">
                            <n-button v-if="activeTabIndex > 0"
                                      wide
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            async nextTab() {
                ...
                if (isValid && this.activeTabIndex < this.tabCount - 1) {
                    this.activeTabIndex++;
                } else if (isValid && this.activeTabIndex < this.tabCount) {
                    this.wizardComplete();
                }
                return isValid;
            },
        ...
        mounted() {
            ...
            var id = this.isset(this.$route.params.id) ? this.$route.params.id : null;

            if (id) {
                this.$nextTick(() => {
                    this.tabs.forEach(function (index, value) {
                        index.checked = true;
                    });
                    this.onResize();
                });
...
<style lang="scss">
    ...
    .bg-hintify {
        background: rgba(0, 0, 0, 0) linear-gradient(to right, #00a1cb 0%, #00c46e 100%) repeat scroll 0 0;
    }
